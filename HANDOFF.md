...
小游戏入口不要只按“70%”一个数字判断。当前 `chapter-027.seed.json` 要求探索度 >=70%，并且 9 个核心线索展签全部已读；这些必读标签权重合计为 89%，UI 应展示“核心线索 9/9”这类状态。

`map-nodes.seed.json` 是 MVP 演示节点，不是全书考据路线。其 `id` 是前端 appNodeId 和本地进度 ID，`canonicalRouteId` 映射到 `route-canon.seed.json` 的正式路线节点。正式大地图应以后续 `route-canon.seed.json` 的全书抽取结果为准。
...
- 展签显示繁体原文和简体白话解释
- 阅读展签累计探索度
- 探索度 >=70% 且核心线索展签全读完后小游戏入口可用
- 完成小游戏后进入静态全景预览、360 全景或动画奖励
- 徽章点亮并保存到本地进度
- 刷新页面后进度不丢失
