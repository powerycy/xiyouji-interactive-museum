...
- `/assets/chapters/027/scenes/baihuling-main-v1.png`
...
- `baihuling-main-v1.png`：热点空间清楚，适合章节探索 demo；唐僧帽子偏华丽，八戒肤色略浅，需后续复核。
...
- `chapter-027-badges-v1.png`：徽章合图可用作视觉参考；`locked-v1` 和 `unlocked-v1` 当前是同一合图副本，正式版需裁切或重生成单枚。

## 开发注意

`chapter-027.seed.json` 已指向 `baihuling-main-v1.png`，但热点坐标仍需在 `/studio` 里按真实图片重新校准。
